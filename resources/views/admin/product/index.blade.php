...
<div>
    ...
    <div class="card">
        <div class="card-header border-0 pt-6">
            <div class="d-flex align-items-center position-relative my-1">
                <input type="text" id="search" name="search" data-table="datatable" autocomplete="off"
                    ...
                    placeholder="{{__('app.panel.search_name', ['name' => str()->plural(__('admin.product'))])}}">
            </div>
        </div>
        <div class="card-body pt-0">
            ...
            <thead>
                    <tr>
                        <th class="min-w-125px removeuppercase">Id</th>
                        <th class="min-w-125px removeuppercase">Name</th>
                        <th class="min-w-125px removeuppercase">Sku</th>
                        <th class="min-w-125px removeuppercase">Price</th>
                        @foreach($competitors as $key => $value)
                        <th class="min-w-125px removeuppercase">{{$value}} Link</th>
                        <th class="min-w-125px removeuppercase">Price</th>
                        @endforeach
                        <!--<th class="min-w-125px">Status</th>-->
                        <th class="min-w-125px removeuppercase">Action</th>
                    </tr>
                </thead>
            </table>
        </div>
...
<script>
    $(document).ready(function () {
        ...
        let table = $('#datatable').DataTable({
            ...
            columns: [
                {data: 'odoo_id', name: 'id'},
                {data: 'name', name: 'name'},
                {data: 'default_code', name: 'default_code'},
                {data: 'list_price', name: 'list_price', render: function (data, type, row) {
                    return `
                ...
                @foreach($competitors as $key => $competitor)
                    {
                        data: 'competitor_ink_{{$key}}', 
                        name: 'competitor_ink_{{ $key }}',
                        orderable: false,
                        searchable: false,
                        render: function(data, type, row) {
                            let displayText = data ? (data.length > 30 ? data.substring(0, 30) + '...' : data) : 'No link available';
                            return `
                                <div class="d-flex align-items-center gap-2">
                                    <a href="javascript:void(0)" 
                    ...
                    {
                        data: 'competitor_price_{{$key}}', 
                        name: 'competitor_price_{{ $key }}',
                    },
                @endforeach
        ...
        $(document).on('click', '#saveCompetitorLink', function() {
            ...
            if (!link.trim()) {
                ...
                $.ajax({
                    url: "{{ route('admin.product.addLink') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
        ...
        $(document).on('click', '.sync-product', function() {
            ...
            $.ajax({
                url: "{{ route('admin.product.sync-specific') }}",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    odoo_id: odooId
